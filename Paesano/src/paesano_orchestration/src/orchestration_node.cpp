...
#include "paesano_interfaces/msg/orchestration_state.hpp"
#include "paesano_interfaces/srv/navigate_to.hpp"
#include "paesano_interfaces/srv/save_map.hpp"
#include "paesano_interfaces/srv/set_mode.hpp"
#include "paesano_interfaces/srv/teleop_command.hpp"
...
class PaesanoOrchestrationNode : public rclcpp::Node
{
public:
  using NavigateTo = paesano_interfaces::srv::NavigateTo;
  using SaveMap = paesano_interfaces::srv::SaveMap;
  using SetMode = paesano_interfaces::srv::SetMode;
  using TeleopCommand = paesano_interfaces::srv::TeleopCommand;
  using Trigger = std_srvs::srv::Trigger;
  using OrchestrationState = paesano_interfaces::msg::OrchestrationState;
  using AStar = paesano_navigation::action::AStar;
  using GoalHandleAStar = rclcpp_action::ClientGoalHandle<AStar>;

  PaesanoOrchestrationNode()
  : rclcpp::Node("paesano_orchestration")
  {
    sim_ = declare_parameter<bool>("sim", false);
    startup_mode_ = declare_parameter<std::string>("startup_mode", "mapping");
    bringup_package_ = declare_parameter<std::string>("bringup_package", "paesano_bringup");
    bringup_launch_ = declare_parameter<std::string>("bringup_launch", "paesano_bringup.launch.py");
    ...
    path_subscription_ = create_subscription<nav_msgs::msg::Path>(
      "/path", path_qos, std::bind(&PaesanoOrchestrationNode::handlePath, this, std::placeholders::_1));
    navigating_subscription_ = create_subscription<std_msgs::msg::Bool>(
      "/is_navigating", 10, std::bind(&PaesanoOrchestrationNode::handleNavigating, this, std::placeholders::_1));

    set_mode_service_ = create_service<SetMode>(
      "/orchestration/set_mode",
      std::bind(&PaesanoOrchestrationNode::handleSetMode, this, std::placeholders::_1, std::placeholders::_2));
    save_map_service_ = create_service<SaveMap>(
      "/orchestration/save_map",
      std::bind(&PaesanoOrchestrationNode::handleSaveMap, this, std::placeholders::_1, std::placeholders::_2));
    navigate_to_service_ = create_service<NavigateTo>(
      "/orchestration/navigate_to",
      std::bind(&PaesanoOrchestrationNode::handleNavigateTo, this, std::placeholders::_1, std::placeholders::_2));
    pause_navigation_service_ = create_service<Trigger>(
      "/orchestration/pause_navigation",
      std::bind(&PaesanoOrchestrationNode::handlePauseNavigation, this, std::placeholders::_1, std::placeholders::_2));
    resume_navigation_service_ = create_service<Trigger>(
      "/orchestration/resume_navigation",
      std::bind(&PaesanoOrchestrationNode::handleResumeNavigation, this, std::placeholders::_1, std::placeholders::_2));
    cancel_navigation_service_ = create_service<Trigger>(
      "/orchestration/cancel_navigation",
      std::bind(&PaesanoOrchestrationNode::handleCancelNavigation, this, std::placeholders::_1, std::placeholders::_2));
    stop_motion_service_ = create_service<Trigger>(
      "/orchestration/stop_motion",
      std::bind(&PaesanoOrchestrationNode::handleStopMotion, this, std::placeholders::_1, std::placeholders::_2));
    teleop_service_ = create_service<TeleopCommand>(
      "/orchestration/teleop",
      std::bind(&PaesanoOrchestrationNode::handleTeleop, this, std::placeholders::_1, std::placeholders::_2));

    teleop_timer_ = create_wall_timer(50ms, std::bind(&PaesanoOrchestrationNode::checkTeleopTimeout, this));
    navigation_state_timer_ = create_wall_timer(
      100ms, std::bind(&PaesanoOrchestrationNode::checkNavigationStateTimeout, this));
    launch_process_timer_ = create_wall_timer(
      500ms, std::bind(&PaesanoOrchestrationNode::checkLaunchProcess, this));
  ...
  ~PaesanoOrchestrationNode() override
  {
    try {
      stopLaunch();
    } catch (...) {
...
private:
  ...
  {
    return sim_ ? "/home/Paesano/ros2_ws/maps/my_map.yaml" :
      "/home/Paesano/ros2_ws_pi/maps/my_map_real.yaml";
  }

  std::string defaultMapSavePrefix() const
  {
    return sim_ ? "/home/Paesano/ros2_ws/maps/my_map" :
      "/home/Paesano/ros2_ws_pi/maps/my_map_real";
  }
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<PaesanoOrchestrationNode>();
