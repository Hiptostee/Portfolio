...
class PaesanoBridgeNode(Node):
    def __init__(self, ros_args: Optional[list[str]] = None) -> None:
        rclpy.init(args=ros_args)
        super().__init__('paesano_mobile_bridge')
        ...
        self.bringup_package = str(self.declare_parameter('bringup_package', 'paesano_bringup').value)
        self.bringup_launch = str(
            self.declare_parameter('bringup_launch', 'paesano_bringup.launch.py').value
        )
        ...
        self.create_subscription(OccupancyGrid, '/map', self._handle_map, state_qos)
        ...
        self.create_subscription(NavPath, '/path', self._handle_path, state_qos)
    ...
    def default_map_yaml_path(self) -> str:
        return '/home/Paesano/ros2_ws/maps/my_map.yaml' if self.sim else '/home/paesano/ros2_ws_pi/maps/my_map_real.yaml'

    def default_map_save_prefix(self) -> str:
        return '/home/Paesano/ros2_ws/maps/my_map' if self.sim else '/home/paesano/ros2_ws_pi/maps/my_map_real'
...
async def telemetry_stream(websocket: WebSocket, bridge: PaesanoBridgeNode) -> None:
...
def create_app(bridge: PaesanoBridgeNode) -> FastAPI:
    @asynccontextmanager
    ...
    app = FastAPI(title='Paesano Mobile Bridge', lifespan=lifespan)
    app.add_middleware(
        CORSMiddleware,
        allow_origins=['*'],
        allow_methods=['*'],
...
def main() -> None:
    bridge = PaesanoBridgeNode(sys.argv)
