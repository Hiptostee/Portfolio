...
This technical report documents the design, implementation, and testing of Paesano, a holonomic autonomous mobile robotics platform. Paesano is intended to navigate indoor environments using probabilistic state estimation, map-based planning, and closed-loop trajectory tracking while being monitored and operated through a mobile application with live telemetry. Both the hardware and software stacks were designed to be modular so that future sensors, estimation methods, or control strategies can be integrated without redesigning the full platform.

The project was developed over two semesters. The first focused on hardware integration, power distribution, and embedded motor-control firmware. The second focused on state estimation, planning, and trajectory tracking. The following sections describe the resulting architecture, the algorithms used in the final system, and the experiments used to evaluate its performance. A central goal of the project was to gain practical experience building a full robotics stack while producing a platform that can continue to support future experimentation.
...
\noindent\textbf{Additional Required Tool}
\begin{itemize}
...
I designed the system around a clear separation between high-level autonomy and low-level motor control. This allows the high-level compute module to run estimation, planning, navigation, and operator-facing services without also carrying the timing burden of direct motor actuation.
...
The robot is powered by a 7.4V nominal 2S 5200mAh LiPo battery. This battery directly powers the entire system, acting as the "heart" 
...
The Pico is powered from a regulated 5V rail within the motor control module so that motor current spikes are less likely to disturb the higher-level compute hardware.

\Needspace{0.7\textheight}
\subsection{Motor Control Stack}
The HiWonder chassis originally came with its own motor control module and PID controller. However, the available hardware was limited and the firmware was not practical to customize for this project. I therefore designed a dedicated motor control module built around the Pico and two TB6612 dual motor drivers.

This motor control module performs closed-loop control of the four DC motors at a fixed update rate of 50Hz. Offloading these tasks to a microcontroller keeps the low-level loop deterministic and allows the Raspberry Pi 5 to reserve its compute resources for higher-level autonomy tasks. Figure~\ref{fig:motor-control-module} shows the schematic of this subsystem.
\begin{figure}[H]
    \centering
...
measurements which are used for mapping and localization. The scan data is produced at approximately 10Hz.
\begin{itemize}
...
The design philosophy of this robot is defined by five ideas:
\begin{itemize}
    \item Modularity
    \item Simplicity
    \item Expandability
...
is handled independently from high-level compute tasks, allowing each layer to be developed and debugged separately.

I also considered cost effectiveness when selecting components, favoring lower-cost parts where possible while still leaving room for future expansion. At the same time, simplicity was prioritized by avoiding unnecessary hardware and architectural complexity. The result is a platform that is practical to develop on, straightforward to modify, and capable of supporting future improvements.


\section{Firmware}
All firmware for this project was written in C++ for compatibility with the RP2040 microcontroller on the Pico and for predictable runtime performance. The firmware is responsible for real-time motor control, encoder processing, and communication with the Raspberry Pi 5. Because motor control requires consistent timing and low latency, this layer operates independently of the higher-level ROS2 stack.
...
In this system, sensor measurements such as wheel encoder data, IMU readings, and LiDAR scans are published on dedicated topics and consumed by estimation nodes that fuse them into a consistent robot state estimate. Higher-level nodes for navigation and trajectory following use that estimate to generate motion commands, which are ultimately published as body-frame velocity commands and sent to the motor control module for actuation.
...
Paesano demonstrates that a modest-cost holonomic platform can support a complete autonomy stack spanning embedded motor control, probabilistic state estimation, map-based planning, trajectory generation, and operator interaction through a mobile application. Architecturally, the project separates deterministic low-level actuation from higher-level computation on the Raspberry Pi 5, which simplified development and made the system easier to debug and extend.

The experimental results support that architecture. At the low level, the drivetrain exhibited repeatable command tracking with small steady-state error, modest overshoot, and short post-stop decay across forward, lateral, and rotational motion. At the system level, the robot followed planned map-frame trajectories with a mean cross-track error of \SI{0.0088}{m}, a mean final position error of \SI{0.0059}{m}, and successful completion of all recorded navigation runs. These results indicate that the estimation, planning, and control layers are operating consistently within the same frame and within the practical limits of the hardware.

The most important future improvements are now incremental rather than architectural. On the hardware side, the platform would benefit from a higher quality LiDAR, a more rigid mechanical structure, and more complete electrical packaging. On the software side, the most promising extensions are improved localization robustness in ambiguous environments, richer navigation behaviors, and broader benchmarking under more varied conditions. Because the current design is modular at both the firmware and ROS2 levels, those future improvements can be integrated without restructuring the full system.
